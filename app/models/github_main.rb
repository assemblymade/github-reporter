class GithubMain
  ...
  def self.push_user_highlights(owner, repo_name, time_length, org_slug, commit_history)
    #commit_history = Githubber.commit_history(time_length, repo_name, owner)
    users = Githubber.top_users_in_history(commit_history)
  ...
  def self.delete_all()
    url = "http://titan-api.herokuapp.com/reporter"
    Remote.delete(url)
  end
